package com.hudhaifa.nightowl.web.rest;

import com.hudhaifa.nightowl.config.KafkaProperties;
...
class VideoStreamingAgentKafkaResourceIT {
    ...
    void setup() {
        ...
        VideoStreamingAgentKafkaResource kafkaResource = new VideoStreamingAgentKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/video-streaming-agent-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/video-streaming-agent-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
